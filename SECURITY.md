...
- **Beyond localhost, it speaks plain HTTP.** Binding another address (`--host`) requires `--allow-remote`, because passwords, tokens and terminal contents then cross the network unencrypted. Do it only on a network you trust, such as a company's internal network, and with a strong password. For more protection, put it behind HTTPS: Tailscale Serve, or a reverse proxy with TLS and its own authentication. The machine's own names and addresses are accepted; pass `--allow-host <name>` for any other name browsers use, such as a DNS alias or a proxy's name. `up` remembers these options.
